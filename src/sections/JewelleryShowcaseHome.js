...
export default function JewelleryShowcaseHome() {
    const { t } = useLanguage();
    ...
    useEffect(() => {
        ...
        if (!track) return;
        const cards = track.querySelectorAll('.jewellery-home-card');
        ...
        <section className="py-24 md:py-32 bg-white">
            <div ref={headingRef} className="px-6 md:px-12 lg:px-16 mb-16 opacity-0">
                <p className="text-gold text-[11px] tracking-[0.3em] uppercase mb-4">
                    {t('jewelleryShowcaseHome.eyebrow')}
                </p>
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <h2 className="font-serif text-display text-charcoal">
                        {t('jewelleryShowcaseHome.heading')}
                    </h2>
                    <Link
                        href="/services#jewellery"
                        className="inline-flex items-center gap-2 text-[12px] tracking-[0.15em] uppercase text-dark-grey hover:text-gold transition-colors duration-300 shrink-0"
                    >
                        {t('jewelleryShowcaseHome.viewAll')}
            ...
            <div className="overflow-x-auto scrollbar-hide">
                <div ref={trackRef} className="flex gap-4 md:gap-6 pl-6 md:pl-12 lg:pl-16 pr-6 md:pr-12 lg:pr-16 pb-4" style={{ minWidth: 'max-content' }}>
                    {JEWELLERY_IMAGES.map((src, i) => (
                        <div key={i} className="jewellery-home-card shrink-0 w-[280px] md:w-[380px] lg:w-[450px]">
                            <div className="relative overflow-hidden aspect-[4/3] group">
                                <img
                                    src={src}
                                    alt={`Jewellery shop design ${i + 1}`}
